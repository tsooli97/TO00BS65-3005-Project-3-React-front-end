...
import { patchById } from "./DataRequester";
...
const PatchId = () => {
  ...
  const handleFormSubmit = async (event) => {
    ...
    const cleanedFormData = cleanObjectForPatch(formData);
    console.log(cleanedFormData);

    const response = await patchById(id, cleanedFormData);
  ...
  const showInfoDiv = () => {
    setShowInfo(
      "Name should be a string, and year should be between 1800 and 2030. The rest of the fields should be string. A starring input won't be included unless both actor and role fields are filled."
    );
    setTimeout(() => {
      setShowInfo("");
    }, 15000);
  ...
  return (
    <div>
      <div className="patch-id-div">
        <label className="label-id">ID: </label>
        <input
          className="patch-id-input"
          name="id"
          value={id}
